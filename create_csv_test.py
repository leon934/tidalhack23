...
def student_question(questions) -> list:
    ...
    for key in questions.keys():
        ...
        if input_answer == questions[key]["Answer"]:
            ...
            print('That is incorrect. The correct answer is ' + questions[key]["Answer"] + ".")
        else:
            answers.append(0)
    return answers
...
def interest_learning():
    ...
    input_hobby = input("What interest do you want the curriculum to relate to? ") # tennis
    input_topic = input("What do you want to learn about? ") # first derivative = 0
    question_amount = input("How many questions do you want to generate? ") # 5
    # first derivative

    print()

    interest_curriculum = interest_questions
        
    for key in interest_curriculum.keys():
        print(key)
        
        for choice in interest_curriculum[key].keys():
            if choice != "Answer":
                print(choice + ": " + interest_curriculum[key][choice])

        input_answer = input("What is your answer? ").upper()
        
        if input_answer == interest_curriculum[key]["Answer"]:
            print('That is correct!')
        else:
            print('That is incorrect. The correct answer is ' + interest_curriculum[key]["Answer"] + ".")
        print()

def main():
    question = []
    columns_to_add = {'name' : 0}
    answer_again, interest_learning_input = "Y", 'Y'
    ...
    topic_df.to_csv('./STUDENT_ANSWERS.csv', index=False)
